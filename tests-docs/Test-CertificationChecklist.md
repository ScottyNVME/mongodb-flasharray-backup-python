...
| **2.A.d** Remove a shard, then restore | 🔲 Topology validated; restore **blocked (OM backup defect)** | `removeShard` drained the shard to 0 chunks; removed from `automationConfig` (v11) → cluster healthy as 4-node/3-shard, `listShards`=3, all agents at goal. **But the post-removal snapshot is blocked by a snapshot job stuck in `FAILING`** (`6a28280e…`): 6 snapshots stalled `PENDING` (cursor never opened), then the failed job wedged in `FAILING` and now OM rejects every new snapshot with `500 THIRD_PARTY_SNAPSHOT_FAILING`. The stuck job survived **all** remediation: `/fail` (400), restart of all 4 MongoDB agents, API `unmanage`/`manage`, a 10-min settle, a full `mongodb-mms` restart, **and** a UI disable/re-enable of backup. No leaked `$backupCursor` on any node; cluster data 100% healthy throughout. Root cause is an OM **appdb-level stuck job** after shard removal → **MongoDB support case**. Restore not exercised. Resume point: once the stuck job clears, take a clean snapshot → restore → verify drift 0. |
| **2.A.g / 2.A.h** Config conversion (embedded ↔ dedicated config server) | 🔲 In progress | **config-00 provisioned**: created 1 TB FA volume `aen-mongo-config-00-data` on sn1-c60-e12-16 → connected to host group `vc01-Workload-Cluster-1` → attached as a physical RDM via PowerCLI → `xfs` mounted at `/data/mongo`. The conversion itself (Option A: add config-00 to the config-server RS, then `transition{To,From}DedicatedConfigServer` + snapshot/restore each direction) is gated on the 2.A.d backup recovery. |
...
- **Shard removal destabilizes OM third-party backup (significant finding):** after `removeShard` + config removal, OM transiently reported `THIRD_PARTY_DISCOVERY_ERROR`, then a snapshot job wedged in `FAILING` that blocks **all** new snapshots (`THIRD_PARTY_SNAPSHOT_FAILING`). It survived every remediation — agent restarts, API `unmanage`/`manage`, full `mongodb-mms` restart, and a UI disable/re-enable of backup — i.e. an OM **appdb-level stuck job** needing MongoDB support to clear. The cluster's *data* and *automation* were healthy throughout; only OM's backup state for the cluster was affected. **This blocks both 2.A.d and 2.A.g/h** (their snapshots hit the same wall).
