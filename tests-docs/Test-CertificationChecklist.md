...
| **2.A.d** Remove a shard, then restore | 🔲 Topology validated; restore **blocked** | `removeShard` drained the shard back to 0 chunks; removed from `automationConfig` (v11) → cluster healthy as 4-node/3-shard, `listShards`=3, all agents at goal. **But the post-removal snapshot persistently stalls** — OM's backup cursor won't open (`PENDING`, 6 attempts), not cleared by job-cleanup, discovery-wait, agent restart, or API `unmanage`/`manage`; no leaked `$backupCursor` found. Restore not yet exercised — needs an OM-side backup-daemon reset. |
...
- **Shard removal destabilizes OM third-party backup:** transient `THIRD_PARTY_DISCOVERY_ERROR` (monitoring/automation reconciliation) followed by a **persistent snapshot-cursor stall** that client-side resets don't clear — the open 2.A.d blocker.
